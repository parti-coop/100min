...
FROM `TABLES` where table_schema like "100min" group by table_schema;

SELECT concat("ALTER TABLE `",table_schema,"`.`",table_name,"` ROW_FORMAT=DYNAMIC;") as _sql FROM `TABLES` where
table_schema like "100min" group by table_schema, table_name;
...
FROM `COLUMNS` where table_schema like "100min" and data_type in ('varchar');
...
FROM `COLUMNS` where table_schema like "100min" and data_type in ('text','tinytext','mediumtext','longtext');
